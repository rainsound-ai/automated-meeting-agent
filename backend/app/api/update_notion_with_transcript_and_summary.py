...
@contextmanager
def meeting_processing_context(meeting: Meeting):
    block_tracker.clear()
    try:
        yield
        ...
        set_summarized_checkbox_on_notion_page_to_true(meeting['id'])

@retry(stop=stop_after_attempt(3), wait=wait_exponential(multiplier=1, min=4, max=10))
async def process_meeting(meeting: Meeting):
    with meeting_processing_context(meeting):
        ...
        upload_transcript_to_notion(transcript_toggle_id, transcription)
